...
export function wait (context, callback, when) {
  const test = function () {
    const delta = when - context.currentTime
    if (delta <= 0) {
      callback()
    } else {
...
const noiseBuffer = offCtx.createBuffer(1, 44100, 44100)
for (let i = 0; i < noiseBuffer.getChannelData(0).length; i++) {
...
export class Voice extends BaseNode {
  ...
  stop (when) {
    when = when || this.context.currentTime
    const t = this.envelope.stop(when)
    this.osc1.stop(t)
    this.osc2.stop(t)
    this._lfo.stop(t)
  }
...
export class Track extends BaseNode {
  ...
  _noteOff (message, force) {
    if (!this._voices.get(message.note)) { return }
    if (!this._sustainOn || force) {
      const voice = this._voices.get(message.note)
      voice.stop()
